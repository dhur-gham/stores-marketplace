    ...
    public function processPayment(Order $order, string $payment_token, array $customer_data): ?array
    {
        ...
        try {
            ...
            $request_data = [
                ...
                'customer_details' => [
                    'name' => mb_substr($customer_data['name'] ?? $order->customer->name, 0, 50),
                    'email' => mb_substr($customer_data['email'] ?? $order->customer->email ?? '', 0, 50),
                    'phone' => mb_substr($customer_data['phone'] ?? $order->customer->phone ?? '', 0, 20),
                    'street1' => mb_substr($order->address ?? '', 0, 100),
                    'city' => mb_substr($order->city?->name ?? '', 0, 50),
